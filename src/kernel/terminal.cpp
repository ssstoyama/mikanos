...
void Terminal::ExecuteLine() {
  ...
  if (strcmp(command, "echo") == 0) {
    ...
    if (!file_entry) {
      PrintToFD(*files_[2], "no such command: %s", command);
      exit_code = 1;
    } else { 
